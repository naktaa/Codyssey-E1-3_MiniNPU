...
- `evidence/simplified-final-run.md`
  - 단순화한 모드 1·2와 중복 없는 크기순 성능표의 최종 실행 결과(사용자 재검증 후 생성)
...
- 상태: 기존 Cross/X 필터 쌍의 순수 2차원·1차원 MAC 측정 코드 구현 완료, 사용자 재측정 전. 기존 검증 포함 결과는 `evidence/ten-run-performance.md`에 과거 기록으로 보존함
...
- 상태: 기존 생성·교체·재사용 흐름의 증거는 확보했으며, 중복 없는 필터 기반 성능표의 최신 출력은 사용자 재검증 전
