...
- 크기별 MAC 호출을 10회 독립 측정
- 3×3, 5×5, 13×13, 25×25 포함
- 파일 읽기·입력·출력과 행렬 검증을 제외하고 순수 MAC 함수 호출 구간 측정
- 크기, 평균 시간(ms), 모집단 표준편차(ms), 연산 횟수 N² 출력
- 실제 측정값을 근거로 O(N²) 설명
...
- `main.py`: 프로그램 실행 진입점
- `benchmark_batch.py`: 공식 결과와 분리한 묶음 MAC 벤치마크
- `src/menu.py`: 메뉴 출력과 모드 선택 흐름
- `src/mini_npu.py`: 2차원·1차원 MAC, epsilon 비교, 판정과 단일 MAC 성능 측정
- `src/manual_mode.py`: 독립 3×3 직접 입력과 생성 N×N 필터 입력 흐름
- `src/data_loader.py`: JSON 읽기, 키 해석, 라벨 정규화와 스키마 검증
- `src/json_mode.py`: JSON 케이스별 판정 흐름
- `src/performance.py`: 중복 없는 크기별 Cross/X 필터의 2차원·1차원 성능 구성
- `src/pattern_generator.py`: N×N Cross/X 필터 생성과 실행 중 생성 데이터 관리
- `src/report.py`: 케이스 결과, 전체 요약과 성능 비교 표 출력
...
- `evidence/one-dimensional-performance.md`
  - 기존 2차원 MAC과 보너스 1차원 MAC의 실제 성능 기록과 비교 분석
- `evidence/manual-nxn-filter-flow.md`
  - 모드 1의 3×3 필터 자동 생성, 판정과 실행 중 저장 필터 선택 결과
- `evidence/ten-run-performance.md`
  - 순수 MAC 측정 전환 전 10회 독립 측정의 2차원·1차원 평균·표준편차와 JSON 최종 집계
- `evidence/simplified-final-run.md`
  - 단순화한 모드 1·2 판정과 필수 네 크기의 10회 성능 결과
- `evidence/performance-repetition-experiments.md`
  - 개별 호출과 묶음 측정의 10·100·1,000회 추가 실험 및 해석
- `evidence/generated-filter-merge.md`
  - 7×7 Cross/X 생성 모양과 실행 중 필터의 모드 2 재사용·크기순 성능 결과
- `evidence/mac-status.png`
  - 성능 측정에 사용한 macOS·CPU·메모리 환경 정보(일련번호 가림)
- 유닛 테스트 증거는 사용자 작업 방침에 따라 생성하지 않음
- 파일 오류의 별도 실행 증거는 사용자 결정에 따라 생성하지 않음
- 크기 불일치의 케이스 단위 FAIL과 이후 분석 계속 흐름은 최신 코드 실행 증거 수집 대기
...
4. 성능 시간은 크기별 Cross/X 필터를 타이머 전에 선택·검증하고 같은 입력으로 10회 워밍업한 뒤, 순수 곱셈·누적만 수행하는 `calculate_mac()` 호출을 10회 따로 측정한 평균과 모집단 표준편차로 정의한다. 워밍업은 측정에서 제외하며 1차원 방식도 검증과 변환을 제외하고 같은 조건을 사용한다.
5. 성능 입력은 3×3 코드 내부 필터, JSON 필터와 실행 중 생성 필터를 재사용하며 별도 N×N 기준 행렬을 만들지 않는다.
...
8. 묶음 측정은 `benchmark_batch.py`에 `10`, `100`, `1000` 중 하나를 위치 인자로 전달해 설정별로 수행하고 공식 미션 결과와 분리한다. 공식·비공식 표는 평균·표준편차·CV·N²·반복 횟수의 같은 형식을 사용하되, 묶음별 호출당 평균의 표준편차는 공식 개별 호출 표준편차와 의미가 다름을 명시한다.
9. `data.json`은 메뉴 2 진입 시 한 번 읽어 같은 모드 안에서 재사용하며, 직접 입력과 생성 메뉴에서는 읽지 않는다.
10. 실행 중 저장소에는 크기별 생성 Cross/X 필터만 보관한다. 사용자 패턴, 직접 입력과 JSON 데이터는 저장소에 넣지 않는다.
11. JSON 필터를 먼저 등록하고 생성 필터는 없는 크기만 추가해 크기당 한 번 측정한다.
12. 모드 1과 모드 2의 성능 결과는 같은 표 출력 함수를 사용하며, 한글의 터미널 표시 폭을 고려해 열을 정렬한다. 모드 1 표 제목은 `MAC 성능`으로 표시한다.
13. 모드 1과 모드 2의 실제 측정 횟수는 `PERFORMANCE_REPETITIONS` 하나를 공유하며 워밍업 횟수는 별도 상수로 유지한다.
14. 모드 2는 케이스별 판정과 전체 요약을 먼저 출력하고 2차원·1차원 MAC 성능표를 마지막에 출력한다.
...
### 보너스 1. 1차원 MAC 최적화 비교

- 2차원 행렬을 행 순서의 1차원 리스트로 변환
- 단일 반복문으로 같은 위치의 값을 곱하고 누적
- 3×3·5×5·13×13·25×25에서 기존 2차원 방식과 같은 10회 독립 측정 후 평균과 표준편차 계산
- 행렬 검증, 변환 시간과 콘솔 출력은 측정에서 제외
- 기존 2차원 실행 기록과 새 1차원 실행 기록을 문서에서 비교·분석
- 상태: 기존 Cross/X 필터 쌍의 순수 2차원·1차원 MAC 10회 재측정과 추가 반복 실험 완료. 최신 필수 결과는 `evidence/simplified-final-run.md`, 추가 실험은 `evidence/performance-repetition-experiments.md`에 기록함

### 보너스 2. N×N Cross/X 패턴 생성기

- 메뉴 3번에서 3 이상의 홀수 N을 입력받아 Cross/X 필터 생성
- Cross는 가운데 한 행·열, X는 두 대각선으로 생성
- 같은 크기의 기존 실행 중 필터는 새 생성 결과로 교체
- 모드 1의 직접 3×3 입력은 생성 저장소와 독립적으로 처리
- 모드 1의 N×N 방식은 생성 필터를 재사용하고 없으면 즉시 생성하며 사용자 패턴과 판정
- 모드 2는 기존 JSON 필터와 생성 필터를 중복 없이 합쳐 필터끼리 측정하고 크기순으로 출력
- 성능 표는 크기·평균 시간·표준편차·CV·N²·반복 횟수를 출력
- JSON 파일에는 생성 결과를 저장하지 않으며 프로그램 종료 시 실행 중 데이터 제거
- 상태: 기존 생성·교체·모드 1 재사용과 새 7×7 필터의 모드 2 재사용·크기순 출력 증거를 확보했으며, JSON과 같은 생성 크기의 중복 제거는 사용자 재검증 전
