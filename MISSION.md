...
- `evidence/ten-run-performance.md`
  - 현재 10회 독립 측정의 2차원·1차원 평균·표준편차와 JSON 최종 집계
...
4. 성능 시간은 패턴과 Cross 필터의 `calculate_mac()` 호출을 10회 따로 측정한 평균과 모집단 표준편차로 정의한다. 1차원 방식도 같은 10회를 사용한다.
...
- 변환 시간과 콘솔 출력은 측정에서 제외
- 기존 2차원 실행 기록과 새 1차원 실행 기록을 문서에서 비교·분석
- 상태: 코드 구현 및 사용자 실행 검증 완료. 10회 독립 측정의 평균·표준편차와 네 필수 크기의 2차원·1차원 결과를 `evidence/ten-run-performance.md`에 기록함
